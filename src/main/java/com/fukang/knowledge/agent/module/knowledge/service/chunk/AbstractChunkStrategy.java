...
public abstract class AbstractChunkStrategy implements ChunkStrategy {
    ...
    public ChunkResult chunk(DocumentParseResult parseResult) {
        if (parseResult == null || parseResult.isEmpty()) {
            log.warn("文档内容为空，无法分块: title={}", parseResult != null ? parseResult.getTitle() : "null");
            throw new BaseException(ErrorCodeEnum.DOCUMENT_CONTENT_EMPTY);
        }

        Metadata metadata = new Metadata();
        if (parseResult.getMetadata() != null) {
            parseResult.getMetadata().forEach(metadata::put);
        }

        Document document = Document.from(parseResult.getContent(), metadata);
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            chunks.add(new ParsedChunk(i, segmentText,
                    ParsedChunk.estimateTokenCount(segmentText), chunkMeta));
        }

        log.info("langchain4j 分块完成: title={}, strategy={}, chunkType={}, totalChunks={}, maxSegmentSize={}, overlap={}",
                parseResult.getTitle(), strategyName(), chunkType, chunks.size(), maxSegmentSize, overlapSize);
        ...
        return new ChunkResult(parseResult.getTitle(), chunks.size(), chunks, strategyName(), chunkMetadata);
    }
     ...
     * 计算分块平均 token 数。
     */
    private double calculateAverageTokenCount(List<ParsedChunk> chunks) {
        if (chunks.isEmpty()) {
            return 0;
        }
        return chunks.stream()
                .mapToInt(ParsedChunk::getTokenCount)
                .average()
                .orElse(0);
    }
}
