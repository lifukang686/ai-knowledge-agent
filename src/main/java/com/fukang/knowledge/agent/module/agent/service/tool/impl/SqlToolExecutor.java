...
public class SqlToolExecutor implements ToolExecutor {
    ...
    public ToolExecutionResult execute(ToolDefinition toolDefinition, Map<String, Object> parameters) {
        ...
        try {
            SqlToolConfig config = parseConfig(toolDefinition.getExecutorConfig());
            String sql = replacePlaceholders(config.getSql(), parameters);

            log.debug("SQL 工具执行: tool={}, sql={}, params={}", toolDefinition.getName(), sql, parameters);
            ...
            long duration = System.currentTimeMillis() - start;
            log.error("SQL 工具执行失败: tool={}", toolDefinition.getName(), e);
    ...
    @NoArgsConstructor
    @AllArgsConstructor
    private static class SqlToolConfig {
        private String sql = "";
    }
}
