...
import com.fukang.knowledge.agent.module.agent.service.tool.LocalMethodToolRegistry;
...
public class LocalMethodToolExecutor implements ToolExecutor {
    ...
    public ToolExecutionResult execute(ToolDefinition tool, Map<String, Object> parameters) {
        ...
        try {
            LocalMethodTool localTool = localMethodToolRegistry.get(tool.getName())
                    .orElseThrow(() -> new IllegalArgumentException("本地方法工具未注册: " + tool.getName()));
            ...
            log.warn("本地方法工具执行失败: tool={}", tool.getName(), e);
