package com.fukang.knowledge.agent.module.knowledge.service.chunk.splitter;
...
public class MarkdownContentOwnershipSplitter implements DocumentSplitter {
    ...
    private void splitSection(Section section, Metadata documentMetadata, List<TextSegment> segments) {
        if (section.getText().length() <= maxSegmentSize) {
            addSegment(section.getText(), section.getTitle(), documentMetadata, segments);
            return;
        }

        String heading = firstHeadingLine(section.getText());
        String body = heading == null ? section.getText() : section.getText().substring(heading.length()).trim();
        ...
        for (String paragraph : paragraphs) {
            ...
            if (candidate.length() > bodyLimit && !current.isEmpty()) {
                addSegment(withHeading(heading, current.toString()), section.getTitle(), documentMetadata, segments);
                current = new StringBuilder(overlapSuffix(current.toString()));
            }
            if (paragraph.length() > bodyLimit) {
                flushCurrent(heading, section.getTitle(), documentMetadata, segments, current);
                splitLongText(heading, paragraph, section.getTitle(), documentMetadata, segments);
            } else {
                if (!current.isEmpty()) {
                    current.append("\n\n");
                }
