...
public class AgentReasoner {
    ...
    public ReasoningResult reason(AgentContext context) {
        ...
        boolean lastStepSuccess = lastStep != null && Boolean.TRUE.equals(lastStep.getSuccess());
    ...
    private ReasoningResult fallbackReason(AgentContext context) {
        ...
        if (Boolean.TRUE.equals(lastStep.getSuccess())) {
            if (hasRemainingSteps) {
                return new ReasoningResult(ReasoningResult.Decision.CONTINUE, "");
            }
        ...
        String error = lastStep.getErrorMessage() != null ? lastStep.getErrorMessage() : "";
    ...
    private String formatStepsHistory(List<AgentStep> steps) {
        ...
        for (AgentStep step : steps) {
            sb.append(String.format("步骤%d [%s]: 工具='%s', 参数=%s\n",
                    step.getStepOrder(),
                    Boolean.TRUE.equals(step.getSuccess()) ? "成功" : "失败",
                    step.getToolName(),
                    step.getParameters()));
            String obs = step.getObservation();
    ...
    private String formatRemainingSteps(List<PlanStep> steps) {
        ...
        return steps.stream()
                .map(s -> String.format("  - 步骤%d: 工具 '%s', 说明: %s",
                        s.getStepOrder(), s.getToolName(),
                        s.getReasoning() != null ? s.getReasoning() : ""))
    ...
    private String formatLastStepDetail(AgentStep lastStep) {
        ...
        String obs = lastStep.getObservation();
        if (obs != null && obs.length() > 300) {
            obs = obs.substring(0, 300) + "...(已截断)";
        }
        return String.format("工具: %s, 耗时: %dms, 错误: %s, 结果: %s",
                lastStep.getToolName(),
                lastStep.getDurationMs() != null ? lastStep.getDurationMs() : 0,
                lastStep.getErrorMessage() != null ? lastStep.getErrorMessage() : "无",
                obs != null ? obs : "(无输出)");
    }
