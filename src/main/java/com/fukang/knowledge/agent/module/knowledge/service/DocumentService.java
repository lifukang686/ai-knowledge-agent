...
import com.fukang.knowledge.agent.module.knowledge.model.vo.DocumentStatusResult;
...
public class DocumentService {
    ...
    public DocumentUploadResult uploadDocument(Long knowledgeBaseId, Long chunkStrategyId, MultipartFile file) {
        ...
        return new DocumentUploadResult(document.getId(), DocumentStatusEnum.PENDING.getCode());
    }
    ...
    public PageResponse<DocumentResult> listDocuments(Long knowledgeBaseId, long page, long pageSize) {
        IPage<DocumentEntity> resultPage = documentMapper.pageByKnowledgeBase(knowledgeBaseId, page, pageSize);
     ...
     * 查询文档，不存在时抛业务异常。
     */
    private DocumentEntity findDocumentById(Long documentId) {
     ...
     * 解析文档状态。
     */
    private String resolveStatus(DocumentEntity document) {
     ...
     * 转换文档列表结果。
     */
    private DocumentResult toDocumentResult(DocumentEntity doc) {
