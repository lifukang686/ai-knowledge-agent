...
public class ServiceDeskKnowledgeQaTool implements LocalMethodTool {
    ...
    public Object execute(Map<String, Object> arguments) {
        ...
        String question = text(arguments, "question", context.getQuestion());
        if (context.getStreamHandler() != null) {
            // SSE 场景复用 RAG 流式链路，把增量 token 直接透传给服务台前端。
            return executeStream(question, context);
        }
        // 非流式调用直接取完整 RAG 结果。
        QaResult result = ragService.answer(question, context.getKnowledgeBaseId(), context.getConversationId());
    ...
    private Object executeStream(String question, ServiceDeskAgentContext context) {
        ...
        ServiceDeskStreamHandler serviceDeskHandler = context.getStreamHandler();

        ragService.answerStream(question, context.getKnowledgeBaseId(), context.getConversationId(), new QaStreamHandler() {
            @Override
        ...
        return toPayload(result, result.getAnswer(), true);
    }
    ...
    private String withNoResultHint(QaResult result) {
        ...
        return "no_results".equalsIgnoreCase(result.getStatus()) ? answer + NO_RESULT_HINT : answer;
    }
    ...
    private Map<String, Object> toPayload(QaResult result, String answer, boolean streamedTokens) {
        return Map.of(
                ...
                "rewrittenQuery", result.getRewrittenQuery() != null ? result.getRewrittenQuery() : "",
