package com.fukang.knowledge.agent.module.modelruntime.service.storage;
...
public class EmbeddingIndexStorageService {
     ...
     * 找到对应的 DocumentChunkEntity，构建 Metadata 后将 Embedding 和 TextSegment
     ...
     */
    public ChunkStorageResult saveVectorsToPgVector(List<DocumentChunkEntity> chunks,
        ...
        if (embeddingResult == null
                ...
                || embeddingResult.getEmbeddings().size() != chunks.size()) {
        ...
        try {
            for (EmbeddingVector embeddingVector : embeddingResult.getEmbeddings()) {
                int chunkOrder = embeddingVector.getChunkOrder();
                if (chunkOrder < 0 || chunkOrder >= chunks.size()) {
                    log.warn("向量顺序号超出块列表范围: chunkOrder={}, chunkListSize={}",
                            chunkOrder, chunks.size());
                    throw new BaseException(ErrorCodeEnum.VECTOR_STORAGE_FAILED);
                }
                DocumentChunkEntity chunk = chunks.get(chunkOrder);
                ...
                Embedding embedding = Embedding.from(embeddingVector.getVector());
