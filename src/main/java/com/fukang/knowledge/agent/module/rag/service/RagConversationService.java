...
import com.fukang.knowledge.agent.module.conversation.service.ConversationMemoryService;
import com.fukang.knowledge.agent.module.memory.service.UserMemoryContext;
import com.fukang.knowledge.agent.module.memory.service.UserMemoryService;
...
public class RagConversationService {
    ...
    public String buildAnswerMemory(ConversationMemoryContext memory, UserMemoryContext userMemory) {
        ...
        if (memory.getSummary() != null && !memory.getSummary().isBlank()) {
            builder.append("会话摘要：").append(memory.getSummary()).append("\n");
        }
        if (memory.getAnswerHistory() != null && !memory.getAnswerHistory().isBlank()) {
            builder.append("最近对话：\n").append(memory.getAnswerHistory()).append("\n");
        }
        if (userMemory != null && userMemory.hasMemory()) {
            builder.append("用户级记忆：\n").append(userMemory.getPromptText());
        }
